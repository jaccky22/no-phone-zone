...
export function ChallengeForm() {
  ...
  const [submittedData, setSubmittedData] = useState<{name: string; email: string; challengeType: ChallengeType} | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setIsSuccess(true);
      setSubmittedData({ name, email, challengeType: selectedChallenge });
  ...
  return (
    <>
      <div className={`w-full max-w-md mx-auto p-6 rounded-xl shadow-lg bg-white dark:bg-gray-800 transition-all duration-500 ${isLoaded ? 'opacity-100 scale-100' : 'opacity-0 scale-95'} gradient-border`}>
        ...
        {isSuccess ? (
          <div className="bg-gradient-to-r from-green-50 to-emerald-50 dark:from-green-900/30 dark:to-emerald-900/30 border border-green-200 dark:border-green-800 text-green-700 dark:text-green-400 px-6 py-5 rounded-xl shadow-md mb-4 animate-fadeIn">
            <div className="flex items-center mb-3">
              ...
              <p className="font-bold text-lg">Challenge Joined Successfully!</p>
            </div>
            <p className="text-sm ml-11">Check your email for further instructions and tips to get started on your journey to reduce phone usage.</p>
            <div className="flex ml-11 mt-5 space-x-3">
              <button 
                onClick={() => {
                  setShowEmailPreview(true);
                }} 
                className="inline-flex h-12 items-center justify-center rounded-md gradient-bg-1 px-6 py-2 text-sm font-medium text-white shadow-lg transition-all duration-300 hover:scale-105 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-400 focus-visible:ring-offset-2"
              >
              ...
              <button 
                ...
                className="inline-flex h-12 items-center justify-center rounded-md gradient-bg-3 px-6 py-2 text-sm font-medium text-white shadow-lg transition-all duration-300 hover:scale-105 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-green-400 focus-visible:ring-offset-2"
              >
      ...
      {/* Email Preview Modal */}
      {showEmailPreview && submittedData && (
        <EmailPreview 
          name={submittedData.name} 
          email={submittedData.email} 
          challengeType={submittedData.challengeType} 
